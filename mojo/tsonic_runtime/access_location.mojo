from .callable import ErasedCallableContext, RaisingCallable, allocate_callable_environment, destroy_callable_environment
from .location_identity import LocationIdentity
from .typed_location import TypedLocation


@fieldwise_init
struct _AccessLocation[Owner: Movable & Deinitable, Key: Copyable & Deinitable, Value: Movable & Deinitable]:
    ...
    @staticmethod
    def load(context: ErasedCallableContext, var _arguments: Tuple[]) raises -> Self.Value:
        var access = context.unsafe_bitcast[Self]()
        return access[].read(access[].owner, access[].key)

    @staticmethod
    def store(context: ErasedCallableContext, var arguments: Tuple[Self.Value]) raises:
        var access = context.unsafe_bitcast[Self]()
        access[].write(access[].owner, access[].key, arguments[0]^)


def access_location[Owner: Movable & Deinitable, Key: Copyable & Deinitable, Value: Movable & Deinitable](
    var owner: Owner,
    key: Key,
    ...
    var environment = allocate_callable_environment(
        Environment(owner^, key.copy(), read, write), destroy_callable_environment[Environment]
    )
